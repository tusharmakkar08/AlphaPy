...
class window():
    def search_dialog(self, widget, data=None):
        ...
        dialog = gtk.Dialog("Find", None, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
    ...
    def create_tab(self, title, d=0):
        ...
        if title != 'Untitled':
            if d:
                self.textbuffer[-1].set_text(zlib.decompress(open(title, "rb+").read()))
            else:
    ...
    def on_closetab_button_clicked(self, sender, widget):
        ...
        if self.change[pagenum] == 1:
            dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                                       gtk.MESSAGE_WARNING, gtk.BUTTONS_YES_NO, "Do you want to save before close ?")
            dialog.set_title("WARNING ..")
            print '\a'
            response = dialog.run()
            dialog.destroy()
    ...
    def compr(self, widget):
        ...
        pg = self.notebook.get_current_page()
        dialog = gtk.FileChooserDialog("Choose file name", None, gtk.FILE_CHOOSER_ACTION_SAVE,
        ...
        if response == gtk.RESPONSE_OK:
            ...
            out = open(file, "wb+")
            out.write((zlib.compress(self.textbuffer[pg].get_text(self.textbuffer[pg].get_start_iter(),
                                                                  self.textbuffer[pg].get_end_iter()))))
            out.close()
    ...
    def changestbr(self, widget, data=None):
        ...
        data = "\t\tline : " + str(cl) + "  col : " + str(col) + "\t\tlines : " + str(ln) + " chars : " + str(
            ch) + "\tSel : " + str(selln) + " | " + str(selch)
    ...
    def find_next(self, widget, data=None):
        ...
        if iput == '':
            return
        start_iter = self.found[pg][1] if self.found[pg] else self.textbuffer[pg].get_iter_at_mark(
            self.textbuffer[pg].get_insert())
    ...
    def find_prev(self, widget, data=None):
        ...
        if iput == '':
            return
        start_iter = self.found1[pg][0] if self.found1[pg] else self.textbuffer[pg].get_iter_at_mark(
            self.textbuffer[pg].get_insert())
